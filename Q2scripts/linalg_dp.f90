...
!
  ...
  real(8) :: A(n,n), p(1,n) , L(n,n) , U(n,n) , b(n,1) , b_temp(n,1) , q(1,n) , residual , temp(n,1) , b_res(n,1) , A_res(n,n)
  integer :: k, j , i , method
  
  
  
	
    do i = 1, n
		p(1 , i) = i;						!!p is permutation matrix for partial pivoting
		q(1 , i) = i;						!!q is permutation matrix for complete pivoting which operates rows
	    b(i , 1) = sqrt(i+0.0);      !!!SQRT(X) where X must be real. i is an integer. So i + 0.0 is real.
	enddo
	b_res = b;    						!!to calculate residuals
	...
	enddo
	A_res = A;							!!to calculate residuals
	...
	!!Let user to choose which method to use.
	print*,"Please choose method: 1 = LU without pivoting; 2 = partial pivoting; 3 = complete pivoting "
    read *, method
	...
	b_temp = b;        					!!permute b matrix for LU factorization 
	do i = 1 , n
	...
	!!LU factorization LOW MEMORY approach
	call SP_UNIT_LSOLVE(n , A , b)
	call SP_USOLVE(n , A , b)
	
	b_temp = b;            			!!here b is result vector x, need to permute if using complete pivoting 
	do i = 1 , n
	...
	!!!! residual norm
	temp = abs(MATMUL(A_res,b) - b_res);   		!!MATMUL function is for matrix multification.
	residual = MAXVAL(temp(:,1))/MAXVAL(abs(b_res(:,1)));   !!MAXVAL is maxmum value of a vector
	print *,'residual = '
	print *, residual;
...
subroutine SP_LU_PP(n, p, A)
	...
	implicit none
    integer, intent(in) :: n
    real(8) :: A(n,n), p(1,n), A_temp(n,n), p_temp(1,n) , A_max
    integer :: k, j , r , i  
		 ...
		 r = k;
			do i = k , n
				if (abs(A(i,k)) > A_max) then
						r = i;
						A_max = abs(A(i,k));     				!!find the maxmum element of the column.
				end if 
			enddo
		if (A(r,k)  == 0) then
			! A is not invertiable
            ! Warn the user and exit
				print *, "Error: A is not invertiable"
				exit
		else
				A_temp = A;											!!swap rows between choosed maxmum element row and diagonal element row
				A(k,:) = A(r,:);
				A(r,:) = A_temp(k,:);
				
				p_temp = p;											!!permute vector
				p(1,k) = p(1,r);
				p(1,r) = p_temp(1,k);
				
				do i = k+1 , n											!!LU-low-memory
						A(i,k) = A(i,k) / A(k,k);
						do j = k+1 , n
...
subroutine SP_LU_CP(n, p, q, A)
	...
	implicit none
    integer, intent(in) :: n
    real(8) :: A(n,n), p(1,n), q(1,n) , A_temp(n,n), p_temp(1,n) , q_temp(1,n) , A_max
    integer :: k, j , r , i , m 
    ...
    do k = 1 , n-1
			...
			do i = k , n
				do j = k , n
						if (abs(A(i,j)) > A_max) then
							r = i;
							m = j;
							A_max = abs(A(i,j));       				!!find the maxmum element in the sub matrix
						end if 
				 enddo
			enddo
		...
		if (A(r,m)  == 0) then
            ...
            ! Warn the user and exit
				...
				A_temp = A;												!!swap columns between choosed maxmum element column and diagonal element column
				A(:,k) = A(:,m);
				A(:,m) = A_temp(:,k);
